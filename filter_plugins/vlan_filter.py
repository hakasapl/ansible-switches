def FILTER_VLANS(vlans, cur_groups = None):
    ...
    for vlan in vlans:
        if cur_groups is not None and "fabrics" not in vlan or not (set(vlan["fabrics"]) & set(cur_groups)):
            continue

        if vlan["id"] in outVLANs:
            # Throw exception
            raise ValueError(f"Duplicate VLAN ID not allowed in the same fabric: {vlan['id']}")
